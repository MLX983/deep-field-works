// Optional real-model test. Costs one explicitly configured call; never targets production files.
import fs from 'node:fs';
import os from 'node:os';
import path from 'node:path';
import assert from 'node:assert/strict';
import { DEFAULT_MODEL, invoke } from './agent.mjs';
...
const runtime = { model: process.env.EDITORIAL_BOUNDARY_MODEL ?? DEFAULT_MODEL, reasoning: process.env.EDITORIAL_BOUNDARY_REASONING ?? 'low' };
const { value: result } = invoke(root,'probe','probe',
  ...
  schema,false,process.env.CODEX_BIN || 'codex',runtime);
